...
1. **背景（都市計画基本図相当）**：`height-coverage`と同じデータソース（`stars.optgeo.org`のOSM planetタイル＋Overture buildingsタイル）を再利用するが、スタイルは全く別物。**塗りは一切使わず線画中心**（塗りはゾーニング層の専有物とする。合成後のスタイルに残る`fill`タイプのレイヤーは`zoning-fill`と、非常に淡い`background`色だけであること — 新しいレイヤーを足すときもこの不変条件を破らない）。
   - 建物：`type: "line"`の輪郭線のみ、無彩色。`sources`フィールドに`provider:"osm"`を含むかどうかで判定（`height-coverage`の`HAS_OSM_SOURCE`フィルタ式をそのまま再利用可能、高さの判定は不要）。**色分けは明度差ではなくダッシュパターン・線幅で行う**（ゾーニングの塗りの上に乗る線なので、明度差は「意図しない別の意味」を持つように見えてしまう）。線の色そのものは主要道路の塗り色と同じ`#ffffff`（このサイトの主張が「基本図データ＝建物形状の充実度そのもの」である以上、建物の外周線は道路網と同じ視覚的重みを持ってよいという判断。以前は水域等と同じ淡いグレーで統一していたが、方針転換した — 経緯は[DECISIONS.md](DECISIONS.md)#4・#12参照）。
   - 水域・土地利用・土地被覆・公園：`fill`ではなく境界線のみ。線の色調はPositron本来の淡いトーンを踏襲する（建物線は上記の通り別扱い。道路の白いケーシングが視覚的な主役であり続けるように、それ以外の背景線画は淡く静かに）。
   - 道路・地名ラベル：Positronスタイルをベースに、線画としての硬質さを保つ。
2. **ゾーニング（前面、半透明の塗り）**：Virgo（ラオス公共事業運輸省のデータ共有基盤）のゾーニングデータをPMTiles化して重ねる。色相はVirgoの実際の配色をそのまま使用。不透明度は固定50〜60%ではなく**ズームに応じて0.4→0.2程度にフェードする**（実装時にZoLaの実コードを参照して修正、経緯は[DECISIONS.md](DECISIONS.md)#7）。ホバーでゾーン名・高さ制限・建蔽率・容積率を表示（値が`0`の場合はVirgoの「未設定」を意味するため`-`表示、DECISIONS.md参照）。クリックはGoogle Street View起動用の8方向コンパスを開く（クリック地点周囲に直接ボタンを配置、吹き出しポップアップは使わない。DECISIONS.md#9・#11・#12参照）。
...
4. 生成したPMTilesをこのリポジトリに置く。**実ファイルの本番配置は`hfu/stars`へPRできない**（`data/.gitignore`が`*.pmtiles`を除外、方針は[CONTRIBUTING.md](https://github.com/hfu/stars/blob/main/CONTRIBUTING.md)）——`config/martin.yaml`の変更だけをPRし、実ファイルはGit外（同一マシンならパス通知だけで足りる）でstarsエージェントに転送してもらう。`overture_buildings`のような巨大ファイル向けの**リモートURL**パターンではなく、`pmtiles_jma_1saibun_hkd`と同じ**ローカルパス**パターンになる（経緯：[DECISIONS.md](DECISIONS.md)#2）
...
- 現在の状態・保留中の作業のスナップショット：[HANDOVER.md](HANDOVER.md)
